...
void BufferPoolManager::FlushAllPages() {
  ...
  for (auto &pair : page_table_) {
    auto promise1 = disk_scheduler_->CreatePromise();
    auto future1 = promise1.get_future();
    disk_scheduler_->Schedule({/*is_write=*/true, pages_[pair.second].data_, /*page_id=*/pages_[pair.second].page_id_, std::move(promise1)});
    future1.get();
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool { 
  ...
  std::unique_lock<std::mutex> lk(latch_);
   ...
   if(page_table_.count(page_id)>0) {
      ...
      // delete the frame from replacer and delete this page from the page table and also add this frame into the free frame list
